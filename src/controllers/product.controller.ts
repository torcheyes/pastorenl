...
export const createProduct = async (productData: Partial<IProduct>): Promise<IProduct> => {
  ...
  export const updateProduct = async (slug: string, updateData: Partial<IProduct>): Promise<IProduct | null> => {
    await dbConnect();
    try {
      ...
      if (updateData.title) {
        ...
        if (newSlug !== slug) {
          ...
          const existingProduct = await Product.findOne({ slug: newSlug, _id: { $ne: currentProduct._id } });
          if (existingProduct) {
...
export const getProductsWithPagination = async (
    ...
    limit: number = 12, 
    sort: string = 'latest'
): Promise<{ products: IProduct[], totalPages: number, currentPage: number }> => {
  await dbConnect();
  try {
...
export const getProductsByBrand = async (brand: string): Promise<IProduct[]> => {
  await dbConnect();
  try {
...
export const getProductsByPriceRange = async (minPrice: number, maxPrice: number): Promise<IProduct[]> => {
  await dbConnect();
  try {
    return await Product.find({ price: { $gte: minPrice, $lte: maxPrice } }).lean();
  } catch (error) {
    console.error(`Failed to get products in price range ${minPrice}-${maxPrice}:`, error);
